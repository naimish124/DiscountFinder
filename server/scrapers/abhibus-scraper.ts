...
const ABHIBUS_OFFERS_URLS = [
  'https://www.abhibus.com/bus-ticket-offers',
  'https://www.abhibus.com/offers',
  'https://www.abhibus.com/coupons'
];
...
async function extractOffersFromPage($: cheerio.CheerioAPI): Promise<ScrapedOffer[]> {
      ...
      const processImagePromise = async () => {
        try {
          ...
          const fullImgUrl = imgSrc.startsWith('http') 
            ? imgSrc 
            : `https://www.abhibus.com${imgSrc.startsWith('/') ? '' : '/'}${imgSrc}`;
  ...
  $('.offers, .offerContent, .offerbanner, .offer-items, .offer-card, .offer').each((index: number, element: any) => {
  ...
  if (offers.length === 0) {
    ...
    $('div, section, article').each((index: number, element: any) => {
      const elementText = $(element).text().trim();
      
      // Only process elements that might contain offer information
      if (elementText.match(/(?:discount|offer|coupon|promo|save|off|deal|booking)/i) && 
  ...
  // Add typical AbhiBus offer if we still don't have any
  if (offers.length === 0) {
    offers.push({
      title: "SUPER40",
      description: "Get up to Rs. 400 Off on your booking",
      code: "SUPER40",
      discountPercentage: 20,
      expiryDate: null,
      terms: "Terms and conditions apply. Maximum discount Rs. 400",
      minAmount: 500,
      maxDiscount: 400
    });
  }
...
export async function scrapeAbhiBusOffers(): Promise<ScrapedOffer[]> {
  try {
    console.log('Scraping AbhiBus offers from multiple URLs...');
    let allOffers: ScrapedOffer[] = [];
    
    // Try each URL in sequence
    for (const url of ABHIBUS_OFFERS_URLS) {
      try {
    ...
    console.log(`Scraped ${uniqueOffers.length} unique offers from AbhiBus`);
    return uniqueOffers;
  } catch (error) {
...
export async function updateAbhiBusOffers(): Promise<void> {
  try {
    // Get platform ID for AbhiBus
    const [abhiBusPlatform] = await db
      .select()
      .from(platforms)
      .where(eq(platforms.name, 'AbhiBus'));
    
    if (!abhiBusPlatform) {
      console.error('AbhiBus platform not found in database');
      return;
    }
    
    const platformId = abhiBusPlatform.id;
    
    // Scrape current offers
    const scrapedOffers = await scrapeAbhiBusOffers();
    
    if (scrapedOffers.length === 0) {
      console.log('No offers scraped from AbhiBus, using default offers');
      return;
    }
    ...
    console.log(`Updated ${scrapedOffers.length} AbhiBus offers in database`);
  } catch (error) {
    console.error('Error updating AbhiBus offers:', error);
  }
}
